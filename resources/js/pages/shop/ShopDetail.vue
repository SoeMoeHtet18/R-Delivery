<template>
    <div class="px-9 py-2 font-lato">
        ...
        <div class="mt-4 shop-related-containers">
            ...
            <div v-if="selectedIndex === 1">
                ...
                <div class="grid grid-cols-4 gap-8 my-4">
                    <div class="bg-soft hover:bg-main-hover border rounded-lg shadow-lg text-center">
                        ...
                        <h5 class="my-6 label-count">{{financialAmounts.totalItemAmount}} MMK</h5>
                    </div>
                    <div class="bg-soft hover:bg-main-hover border rounded-lg shadow-lg text-center">
                        <h5 class="my-4 label">MARKUP DELIVERY FEES</h5>
                        <hr class="border-main">
                        <h5 class="my-6 label-count">{{financialAmounts.totalMarkUpDeliveryFees}} MMK</h5>
                    </div>
                    <div class="bg-soft hover:bg-main-hover border rounded-lg shadow-lg text-center">
                        <h5 class="my-4 label">DELIVERY FEES</h5>
                        <hr class="border-main">
                        <h5 class="my-6 label-count">{{financialAmounts.totalDeliveryFees}} MMK</h5>
                    </div>
                    <div class="bg-soft hover:bg-main-hover border rounded-lg shadow-lg text-center">
                        <h5 class="my-4 label">PAY AMOUNT TO SHOP</h5>
                        <hr class="border-main">
                        <h5 class="my-6 label-count">{{financialAmounts.totalAmountToPayShop}} MMK</h5>
                    </div>
                </div>
            ...
            <div v-if="selectedIndex === 2">Selected: PICKUP Content</div>
            <div v-if="selectedIndex === 3">Selected: EXCHANGE Content</div>
            <div v-if="selectedIndex === 4">Selected: PAYMENT Content</div>
            <div v-if="selectedIndex === 5">Selected: TRANSACTION Content</div>
            <div v-if="selectedIndex === 6">Selected: CANCEL Content</div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async getShopCancelOrders() {
            const response = await fetch(`/api/shops/${this.id}/canceled-orders`);
            const data = await response.json();
            this.cancelOrders = data.data;
        },
        ...
        async getShopPayments() {
            const response = await fetch(`/api/shops/${this.id}/paymwnts`);
            const data = await response.json();
        ...
        async getShopOrders() {
            const response = await fetch(`/api/shops/${this.id}/orders`);
            const data = await response.json();

            const formattedData = data.data.map((order, index) => ({
                    'id': order.id,
                    ...
                    'markupDeliveryFees': order.markupDeliveryFees ?? '',
    ...
    computed: {
        joinedDate() {
            const date = new Date(this.shopInfo.created_at);
...
.shop-order-data-grid.custom-data-grid .dx-datagrid-headers .dx-datagrid-table .dx-header-row .dx-datagrid-action .dx-datagrid-text-content {
    white-space: normal;
}
